...
export function GeneratorSection({ t, initialRepoUrl = '', initialToken = '', initialName = '', initialEmail = '' }: GeneratorSectionProps) {
    ...
    const [copied, setCopied] = useState<'remote' | 'config' | null>(null);
    ...
    const getRemoteCommand = () => {
        ...
        try {
            ...
            return `git remote set-url origin https://${token ? token + '@' : ''}github.com/${cleanPath}`;
        } catch (e) {
            return t.invalidUrl;
        }
    };
    ...
    const handleCopy = (type: 'remote' | 'config', text: string) => {
