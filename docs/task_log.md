...
### 📝 Phase 6: 터미널 푸시 경험 개선 (Terminal Guide)
- **Remote 명령어 변경**: 원격 저장소 재정의를 방지하기 위해 `set-url` 생성 로직을 `add` 로직으로 변경.
- **원스톱 푸시 가이드 (Git Push)**: 깃허브 명령어에 익숙하지 않은 사용자를 배려해 클립보드로 한 번에 복사 후 적용 가능한 통합 명령어 모음 제공 (`git add . && git commit ...`).
- **단계별 푸시 가이드 (Step-by-step)**: 원스톱 명령어 외에도 초보자가 각 과정(Staging, Commit, Branch, Push)의 역할을 이해하고 하나씩 따라 칠 수 있도록 4단계 분할 가이드 및 개별 복사 버튼 추가.
